"""Scoped Goal absolute decoder workflow. GPU stages require --execute.
...
CONFIG=ROOT/'configs/vae/libero_goal_all_absolute_decoder_wan21_bs2_ga4_lr1e-5_ep2.json'
CACHE=ROOT/'data/libero_goal_rothko_all_absolute_2cam224_wan21_bf16_h16_latents'
PREP=ROOT/'evaluate_results/libero/goal_absolute_decoder_preparation_padding_20260919'
...
class Inputs:
    def __init__(self,config=CONFIG):
        ...
        assert self.args.suites==['libero_goal'] and self.args.dataset_roots is None
        ...
        assert (len(self.train),len(self.val),len(self.refs),len(self.val_windows))==(413,20,50445,200)
        ...
        self.root=str((Path(self.args.data_root)/core.SUITE_DIRS['libero_goal']).resolve())
        records=[json.loads(x) for x in (Path(self.root)/'meta/episodes.jsonl').read_text().splitlines()]
        self.offsets={};count=0
        for ep in sorted(records,key=lambda x:x['episode_index']):
            self.offsets[ep['episode_index']]=(count,ep['length']);count+=ep['length']
        assert count==52895
        self.contract=build_dataset_contract(dataset_dirs=[self.root],dataset_length=count,num_frames=17,
        ...
        self.manifest=dict(train_episodes=[dataclasses.asdict(e) for e in self.train],
            ...
            val_windows=[dict(episode=w.episode.episode_index,start=w.start) for w in self.val_windows],
            train_cache_indices=[self.index(r) for r in self.refs],train_windows=len(self.refs),steps_per_epoch=789,total_steps=1578,warmup_steps=78,
        ...
        assert len(set(self.manifest['train_cache_indices']))==50445
        self.identity=digest_obj(self.manifest)
        self.reader=None

    def index(self,ref):
        assert ref.episode.dataset_root==self.root
        offset,length=self.offsets[ref.episode.episode_index]
    ...
    def open_cache(self):
        ...
        assert not m.get('benchmark_only') and len(self.reader)==52895
...
def audit(inputs,output):
    ...
    for pool in [inputs.train,inputs.val]:
        for task in sorted({e.task for e in pool}):
            ep=next(e for e in pool if e.task==task)
            for start in sorted({0,max(ep.length-16,0),ep.length-1}):
                ...
                probes.append(dict(episode=ep.episode_index,start=start,exact=True))
...
def evaluate(inputs,vae,store,episodes,stats,args,device,dtype):
    ...
    result=dict(future_position_mae_m=pe[valid].mean().item(),future_rotation_mean_deg=re[valid].mean().item(),future_gripper_mae=ge[valid].mean().item(),
        num_windows=200,num_eval_episodes=20,decode_mode='legacy',anchor_alpha=0,precision='BF16 deployment decoder',manifest_sha256=inputs.identity,
        metrics={})
    for horizon in [8,16]:
...
def train(inputs,resume):
    ...
    if resume:
        ...
        assert saved['metadata']['goal_absolute_identity']==inputs.identity
    ...
    def init_wandb(args,metadata,rank):
        metadata.update(raymap_representation=inputs.codec.representation,pose_mode='all_absolute',
            goal_absolute_identity=inputs.identity,cache_metadata_sha256=inputs.cache_sha,
...
def main():
    ...
    if a.stage in ['audit','train'] and not a.execute:
        command=('python scripts/prepare_goal_absolute_decoder.py audit --execute' if a.stage=='audit' else
            'torchrun --standalone --nproc_per_node=8 scripts/prepare_goal_absolute_decoder.py train --execute')
    ...
    if a.stage=='prepare':
        ...
        print(json.dumps(dict(identity=inputs.identity,train_windows=len(inputs.refs),steps=1578,eval_windows=200,cache_exists=CACHE.exists())));return
    inputs.open_cache()
