...
import torch
from fastwam.representations.vlabench_rothko import VLABenchRothkoCodecConfig,VLABenchRothkoCodec
...
class AllAbsoluteTest(unittest.TestCase):
    def setUp(self):
        self.cfg=VLABenchRothkoCodecConfig(image_height=32,image_width=96,tile_height=32,tile_width=32,horizontal_copies=3,boundary_margin=2,outer_margin=2,frame0_pose_mode='absolute',absolute_position_min=(-1.,-1.,-1.),absolute_position_max=(1.,1.,1.))
        codec=VLABenchAllAbsoluteRothkoCodec(self.cfg)
        self.stats=RothkoNormStats(lo=torch.full((1,3,32,96),-1.),hi=torch.ones(1,3,32,96),metadata=codec.metadata())
        self.codec=VLABenchAllAbsoluteRothkoCodec(self.cfg,self.stats)
    ...
    def test_framewise_encoding_and_duplicates(self):
        ...
        mixed=VLABenchRothkoCodec(self.cfg,self.stats_without_identity())
        expected=mixed.encode(self.pose.reshape(-1,1,7),self.grip.reshape(-1,1,1)).reshape(2,17,3,32,96).permute(0,2,1,3,4)
    ...
    def test_reject_wrong_stats_bounds_and_decoder(self):
        with self.assertRaises(ValueError):VLABenchRothkoCodec(self.cfg,self.stats)
        with self.assertRaises(ValueError):VLABenchAllAbsoluteRothkoCodec(self.cfg,self.stats_without_identity())
        with self.assertRaises(ValueError):VLABenchAllAbsoluteRothkoCodec(replace(self.cfg,absolute_position_max=(2.,1.,1.)),self.stats)
        with self.assertRaises(ValueError):VLABenchAllAbsoluteRothkoCodec(self.cfg,self.stats,decode_mode='robust_joint')
